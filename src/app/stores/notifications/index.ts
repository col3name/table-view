...
import {Notification, NotificationId, NotificationKind} from "../../shared/typings/notifications";
...
export const NotificationStore = types
    ...
    .views((self) => ({
        get notificationsList(): Notification[] {
            return self.notifications.map((item: MNotificationModel): Notification => ({
                id: item.id,
    ...
    .actions((self) => ({
        ...
        push: (
            notification: SomePartial<Omit<Notification, 'id'>, 'timeout' | 'closable'>,
        ) => {
